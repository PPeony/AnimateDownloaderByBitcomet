    ...
    def monitor_file(task: AnimeTask, file_path: str):
        ...
        while task.timer < task.max_timer and not task.stop_event.is_set():
            ...
            if check_done():
                ...
                log(f"✅ {task.row.cells[0].content.value} 下载完成")
                break
        else:
            if not check_done():
                log(f"⏰ {task.row.cells[0].content.value} 下载超时或未完成")
            ...
            while elapsed < timeout:
                ...
                if os.path.exists(result_path) and os.path.getsize(result_path) > 0:
                    try:
                        ...
                        log("✅ AI 返回结果已获取")
                        log(f"🔍 结果：{json.dumps(result_data, ensure_ascii=False, indent=2)}")

                        # 删除文件
                        os.remove(result_path)
                        log("🗑️ result.json 已删除")

                    except Exception as e:
                        log(f"❌ 读取 result.json 失败：{str(e)}")
                    finally:
                        break
            else:
                log("⏰ AI 任务超时，未生成 result.json")
